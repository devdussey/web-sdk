...
export const bookEventHandlerMap: BookEventHandlerMap<BookEvent, BookEventContext> = {
	...
	coinCollectInfo: async (bookEvent: BookEventOfType<'coinCollectInfo'>) => {
		stateGame.coinTotal += bookEvent.spinCoinWin;
		eventEmitter.broadcast({ type: 'soundOnce', name: 'sfx_multiplier_landing' });
		eventEmitter.broadcast({ type: 'tumbleWinAmountShow' });
		await eventEmitter.broadcastAsync({
			type: 'tumbleWinAmountUpdate',
			amount: stateGame.coinTotal,
			animate: true,
		});
		// TODO(frontend art): once Coin ("C") symbol art + a dedicated coin-tally UI
		// component exist, replace the tumbleWinAmount reuse above with real per-tile
		// coin-collect animations at bookEvent.positions.
	},
	iceFreezeInfo: async (bookEvent: BookEventOfType<'iceFreezeInfo'>) => {
		stateGame.frozenIce = bookEvent.positions;
		if (bookEvent.frozeNew) {
			eventEmitter.broadcast({ type: 'soundOnce', name: 'sfx_multiplier_landing' });
		}
		stateGame.subzeroLives = bookEvent.lives;
		// TODO(frontend art): once Ice ("I") symbol art + a lives-indicator UI component
		// exist, replace the sound-only feedback above with a real freeze animation at
		// the newly-landed positions and a visible lives counter.
	},
	iceWinInfo: async (bookEvent: BookEventOfType<'iceWinInfo'>) => {
		if (bookEvent.totalWin > 0) {
			eventEmitter.broadcast({ type: 'soundOnce', name: 'sfx_multiplier_win' });
			await eventEmitter.broadcastAsync({
				type: 'tumbleWinAmountUpdate',
		...
		// TODO(frontend art): once Ice ("I") symbol art exists, replace this with a real
		// "collect all frozen tiles" reveal animation before the round's win events fire.
	},
	// customised
